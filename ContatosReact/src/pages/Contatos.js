...
function Contatos() {
  ...
  useEffect(() => {
    if (selectedUser) {
      const user = usuarios.find((u) => u.email === selectedUser);
      if (user) {
        setPasswordUser(user.senha);
        buscarContatos();
      }
    }
  }, [selectedUser]);
  ...
  const buscarUsuarios = async () => {
    try {
      ...
      if (response.data.length > 0) {
        ...
        setPasswordUser(response.data[0].senha);
      }
    } catch (error) {
  ...
  const buscarContatos = async () => {
    ...
    try {
      const response = await api.post("/contatos/listar", {
        email: selectedUser,
        senha: passwordUser, // Você pode precisar ajustar isso dependendo da sua API
      });
  ...
  const adicionarContato = async (e) => {
    ...
    try {
      const response = await api.post(
        `/contatos/cadastrar/${selectedUser}/${passwordUser}`,
        novoContato
      );
  ...
  const deletarContato = async (celular) => {
    try {
      await api.delete(`/contatos/deletar/${celular}`, {
        data: { email: selectedUser, senha: passwordUser },
      });
      ...
      <UserSelector
        ...
        onChange={(e) => {
          const usuarioSelecionado = usuarios.find(
            (usuario) => usuario.id === parseInt(e.target.value) 
          );

          if (usuarioSelecionado) {
            setSelectedUser(usuarioSelecionado.email);
            setPasswordUser(usuarioSelecionado.senha);

            console.log("Email:", usuarioSelecionado.email);
            console.log("Senha:", usuarioSelecionado.senha);
          }
        }}
      >
            ...
            <ContactItem key={contato.id}>
              ...
              <ContactInfo>
                ...
                {contato.endereco && typeof contato.endereco === "object"
                  ...
                  : contato.endereco || "Endereço não disponível"}
              </ContactInfo>
        ...
        <ModalBackdrop onClick={() => setIsModalOpen(false)}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            ...
            <InputEndereco
              ...
              onChange={handleInputChange}
            />
            <ButtonSalvar type="button" onClick={salvarEndereco}>
              Salvar
            </ButtonSalvar>
          </ModalContent>
        </ModalBackdrop>
